...
private:
	...
	bool canAllocate(int required_frames, int index) override {
		// Check memory vector if frame is unoccupied
...
public:
	PagingAllocator(int max_overall_mem, int mem_per_frame) {
		this->max_overall_mem = max_overall_mem;
		this->mem_per_frame = mem_per_frame;
		this->num_paged_in = 0;
		this->num_paged_out = 0;
	...
	int getNumPagesIn() {
		return this->num_paged_in;
	}

	int getNumPagesOut() {
		return this->num_paged_out;
	}
};