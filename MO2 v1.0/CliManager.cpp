...
CliManager::CliManager()
{
	...
	while (true) {
		...
		if (command == "initialize" && !initialized) {
			...
			if (layout.reportutil_Screen(processes, scheduler->getNumCpu(), scheduler->getActiveCores())) {
		...
		else if (command == "vmstat" && initialized && threadCreated) {
			string information = scheduler->getVmstatInfo();
			layout.vmstatScreen(tokenizer(information));
		}
		else if (command == "process-smi" && initialized && threadCreated) {
			string information = scheduler->getProcess_smiInfo();
			layout.process_smi_Screen(tokenizer(information));
		}
...
void CliManager::screenCommand(vector<string> commandArgs)
{
	...
	if (commandArgs.size() == 3 && commandArgs[1] == "-r") {
		if (processes.find(commandArgs[2]) != processes.end()) {
			// TO DO: Process should not be accessible anymore when it is already finished executing.
			layout.clearScreen();
		...
		layout.screen_ls_Screen(processes, scheduler->getNumCpu(), scheduler->getActiveCores());
	}
	else {
		cout << MAG << "Error: Command not recognized. Aborting." << endl;
	}
