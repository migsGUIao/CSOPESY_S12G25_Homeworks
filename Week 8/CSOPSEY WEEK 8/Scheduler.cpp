...
Scheduler::Scheduler(string filename) {
	...
	this->mem_per_proc = stoi(tokens[9]);
...
void Scheduler::startSchedule()
{
	...
	memAlloc = new MemoryAllocator(max_overall_mem, mem_per_frame);
	/*this->memAlloc = make_unique<MemoryAllocator>(max_overall_mem, mem_per_frame);*/
	this->cpuCycles = 0;
...
void Scheduler::cpuClock()
{
	...
	while (true) {
		...
		if (cpuCycles % quantum_cycles == 0) {
			memAlloc->printMemory(getCurrentTime(), qq, mem_per_proc);
			qq++;
		}
...
void Scheduler::roundRobin(int cpuID)
{
	...
	while (true) {
...

		{
			...
			memAlloc->allocate(mem_per_proc, currentProcess->getName()); // TO DO: Turn this into a condition
			...
			memAlloc->deallocate(mem_per_proc, currentProcess->getName());
