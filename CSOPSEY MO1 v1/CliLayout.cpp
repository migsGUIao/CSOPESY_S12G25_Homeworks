...
void CliLayout::screen_ls_Screen(unordered_map<string, shared_ptr<Process>> processes, int totalCpu, bool status)
{
	int cores_used;
	int cores_free;
	float cpu_util;
	if (status) {
		cores_used = totalCpu;
		cores_free = totalCpu - cores_used;
		cpu_util = static_cast<float>(cores_used * 100.0 / totalCpu);
	}
	else {
		cores_used = 0;
		cores_free = totalCpu;
...
bool CliLayout::reportutil_Screen(unordered_map<string, shared_ptr<Process>> processes, int totalCpu, bool status)
{
